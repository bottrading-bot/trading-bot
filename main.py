# =========================
# 🚀 PRO AUTO TRADING BOT (FINAL + AI FIX)
# =========================
...
ALPACA_API_KEY = "PKMLIK4ZRPDK5VDHO6IQIEGYR6"
ALPACA_SECRET_KEY = "3P1KLayX9hA8M6itc9ecEnpdThjMFYjWD9NKqsUZ7DWk"
...
def get_or_train_model(symbol, df):
    ...
    if symbol not in model_cache or now - last_train_time.get(symbol, 0) > TRAIN_INTERVAL:
        ...
        model.fit(X_scaled, y, epochs=2, verbose=0)

        model_cache[symbol] = (model, scaler)
        last_train_time[symbol] = now

        print(f"🧠 Model neu trainiert: {symbol}")
...
def trade():
    ...
    if not market_ok():
        print("⛔ Markt leicht schwach – keine Trades")
        return
